...
//            Sticker currentSticker = findHandlingSticker();
//            if(null != currentSticker && currentSticker instanceof  TextSticker){
...
//                }
    ...
    public void postScale( float scaleFactor, float focusX, float focusY) {
        Log.d(TAG, "scale: scaleFactor" + scaleFactor);
        Log.d(TAG, "scale: focusX" + focusX);
        Log.d(TAG, "scale: focusY" + focusY);
        if (stickers.size() > 0) {
            for (int i = 0 ; i<stickers.size();i++){
                Sticker sticker = stickers.get(i);
    ...
    public void postTranslate( float velocityX, float velocityY){
        if (stickers.size() > 0) {
            for (int i = 0 ; i<stickers.size();i++){
                Sticker sticker = stickers.get(i);
                if (!sticker.isRelatedPtohoview()){
    ...
    private float getProportion(float totle,float section){
        return section/totle;
    }

    private float getDistance(float now,float should){
            return should-now;
    }
